...
- Reported: Google search results showed the default Vercel/Next.js triangle logo instead of the real brand. **Fixed** — the untouched scaffold `favicon.ico` was deleted. First pass used a generated "U" monogram (`ImageResponse`, same technique as `opengraph-image.tsx`); owner asked for the *actual* logo instead, so that was replaced with `app/icon.png` and `app/apple-icon.png` — static PNGs auto-cropped from `public/logo.jpg`'s shield/planet emblem (top ~310px of the 640×641 source, text wordmark excluded) via a one-off PowerShell/System.Drawing script, not checked into any build tooling. Regenerate by re-running that crop if the logo file ever changes. Note this only controls what *this site* declares — Google can take days/weeks to re-crawl and refresh the icon it displays in results; there's no way to force that faster.
